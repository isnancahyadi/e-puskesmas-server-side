...
<?= $this->section('title') ?>
<title>Tambah Data Dokter &mdash; e-Puskesmas</title>
<?= $this->endSection() ?>
...
<section class="section">
    <div class="section-header">
        <div class="section-header-back">
            <a href="<?= site_url('dokter/tampil') ?>" class="btn"><i class="fas fa-arrow-left"></i></a>
        </div>
    ...
    <div class="section-body">
        <div class="card">
            <div class="card-header">
                <h4>Buat Data Dokter Baru</h4>
            </div>
            <div class="card-body col-md-6">
                <?php $validation = \Config\Services::validation() ?>
                <?php $errors = session()->getFlashdata('errors') ?>
                <form action="<?= site_url('dokter/store') ?>" method="POST" autocomplete="off">
                    <?= csrf_field() ?>
                    <div class="form-group">
                        <label>No. Surat Izin Praktik *</label>
                        <input type="text" name="sip" value="<?= old('sip') ?>" class="form-control <?= isset($errors['sip']) ? 'is-invalid' : null ?>" placeholder="SIP">
